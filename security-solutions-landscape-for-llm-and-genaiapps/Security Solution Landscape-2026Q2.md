...
![Release.png](:/4e90703b75454ba5a9da8b90802e8825)
...
![Deploy.png](:/557f356a448046bd87f8c69f67246b9e)
...
<tr>
   ...
   <td><ul>
		...
		<li>実行時の自己防御</li>
		<li>セキュリティが確保された出力処理</li>
	   <li>エージェント　チェーンにおける異常検知</li>
	   <li>実行時のエージェント ポリシー検証</li>
   </ul></td>
</tr>
</table>
</div>
...
この段階では、パフォーマンス、セキュリティ、およびユーザー インタラクションのリアルタイム監視に焦点が置かれています。主要な指標を追跡することで、異常を検出し、コンポーネントが期待どおりに動作していることを確認し、コンプライアンスを維持します。データは改善のために収集され、問題は事前に解決され、アプリケーションのライフサイクル全体を通して安定性、セキュリティ、および効率性が維持されます。
...
<tr>
	<td><ul>
		<li>新しいデータに基づいた再学習プロセスの自動化</li>
		<li>モデルのドリフトや劣化の検知と対応</li>
		<li>モデルのバージョン管理と、必要に応じたロールバック</li>
		<li>モデルのパフォーマンスの監視（例: レイテンシ、精度、ユーザー操作）</li>
   </ul></td>
   <td><ul>
		<li>敵対的入力の検知</li>
		<li>モデルの動作の分析</li>
