...
#### 質問: RAG（Retrieval Augmented Generation）を使用していますか？ 

回答が「はい」の場合、自組織の検索リポジトリがモデルの挙動を決定する役割を果たします。これは以下のことを意味します。
...
- 回答が「はい」の場合: ユーザーがデータ（指示ではなく）を使用してモデルを欺き、誤った判断をさせようとする[回避攻撃](https://owaspai.org/goto/evasion/)から保護します。ここでは、リスクのレベルを評価することが重要です（以下を参照）。回避攻撃のリスクは許容範囲となる可能性があります。
...
#### 質問: モデルの学習/微調整は自組織で行っていますか？

- 回答が「はい」の場合、学習データは機密情報ですか？ 機密情報の場合、以下の点から保護する必要があります。
	- [モデル出力における不要な情報漏洩](https://owaspai.org/goto/disclosureuse/)
	- [モデル反転](https://owaspai.org/goto/modelinversionandmembership/)
	- [開発環境からの学習データの漏洩](https://owaspai.org/goto/devdataleak/)
	- [メンバーシップ推論](https://owaspai.org/goto/modelinversionandmembership/) - ただし、学習データに何かまたは誰かが含まれていたという事実が機密情報を構成している場合のみ。例えば、犯罪歴を予測するために、学習データセットが犯罪者とその履歴で構成されている場合などです。そのセットのメンバーシップは、その人物が有罪判決を受けた、または犯罪容疑者であることを示します。

#### 質問: RAG を使用していますか？

- 回答が「はい」の場合: 上記の方法を、学習データセットの一部であるかのように拡張データに適用します。リポジトリ データはモデルに入力されるため、出力にも含まれる可能性があります。

モデルの学習/微調整を自組織で行わない場合、学習データ内の不要なコンテンツについては、モデルのサプライヤーが責任を負います。これには、汚染済みデータ（上記参照）、機密データ、または著作権で保護されたデータが含まれます。これらの事項については、ライセンス、保証、契約を確認し、状況に応じてリスクを受容することが重要です。

### モデル盗難に関するリスクの特定

#### 質問: モデルの学習/微調整は自組織で行っていますか？

- 回答が「はい」の場合、モデルは知的財産とみなされますか？その場合、以下の行為から保護する必要があります。
	- [使用時のモデル盗難](https://owaspai.org/goto/modeltheftuse/)
	- [開発時のモデル盗難](https://owaspai.org/goto/devmodelleak/)
	- [ソースコード/設定の漏洩](https://owaspai.org/goto/devcodeleak/)
	- [実行時のモデル盗難](https://owaspai.org/goto/runtimemodeltheft/)

### 入力データの漏洩に関するリスクの特定

#### 質問: 入力データは機密情報ですか？

- [入力データの漏洩](https://owaspai.org/goto/leakinput/)を防止します。特にモデルをサプライヤーが運用している場合は、データが最小限に抑えられ、安全に転送または保管されるよう、適切な注意を払う必要があります。サプライヤー側でログ記録や監視を無効にするオプションなど、サプライヤーが提供するセキュリティ対策を確認してください。ほとんどのクラウド AI モデルでは、入力と出力が（Google Suite や Microsoft 365 の文書と同様に）インフラストラクチャ内で暗号化されていない状態になっていることに注意してください。適切なライセンスと設定を使用すれば、入力と出力の保管や分析を防ぐことができます。残るリスクの1  つは、サプライヤーの政府が召喚状への対応として入力と出力を保管・保持せざるを得なくなる可能性があることです。RAG システムを使用している場合は、取得してプロンプトに挿入するデータも入力データとしてカウントされることに注意してください。これには、多くの場合、機密性の高い企業情報や個人データが含まれます。

### さらなるリスクの特定

#### 質問: モデルはテキスト出力を作成しますか？

- 例えば、モデルの出力をウェブ サイトに表示する際に、その出力に悪意のある JavaScript が含まれている場合など、[セキュアでない出力処理](https://owaspai.org/goto/insecureoutput/)から保護する必要があります。

[悪意のあるユーザーによるモデルの利用不能](https://owaspai.org/goto/denialmodelservice/)（例: 大量の入力、多数のリクエスト）に対する保護も必ず実施してください。モデルをサプライヤーが運用している場合は、既にこの問題に対処するための対策が講じられている可能性があります。

AI システムはソフトウェア システムであるため、このセクションで説明する AI 固有の脅威と管理策に加えて、適切な従来のアプリケーション セキュリティと運用セキュリティが必要です。

## 2. 発生可能性と影響度の推定によるリスク評価


リスクの重大性を判断するには、リスクの発生可能性を評価し、リスクが顕在化した場合の潜在的な影響を評価する必要があります。

### 発生可能性の推定

AI リスクの発生可能性と影響度を推定するには、対象となる AI システムの技術的側面と状況的側面の両方を徹底的に理解する必要があります。AI システムにおけるリスク発生の可能性は、AI アルゴリズムの複雑さ、データの品質とソース、導入されている従来のセキュリティ対策、敵対的攻撃の可能性など、いくつかの要因によって左右されます。例えば、公開データを処理する AI システムは、データ汚染攻撃や推論攻撃の影響を受けやすく、こうしたリスクが発生する可能性が高くなります。金融機関の AI システムは、公開信用スコアを用いて融資審査を行っていますが、データ汚染攻撃の脅威にさらされています。これらの攻撃は信用力評価を操作し、誤った融資判断につながる可能性があります。

確率評価に関わる側面の例は、以下のとおりです。

- 攻撃者のアクセス機会（OWASP、FAIR - 情報リスク要因分析）
- 捕捉リスク（FAIR）
- 能力／ツール／予算（ISO/IEC 27005、OWASP、FAIR）
- システムの感受性（ISO/IEC 27005、FAIR）
- 動機（OWASP、FAIR、ISO/IEC 27005）
- 潜在的な攻撃者の数（OWASP）
- インシデントおよび攻撃試行に関するデータ（ISO/IEC 27005）

### 影響の評価

AI システムにおけるリスクの影響評価には、脅威が顕在化した場合の潜在的な影響を理解することが含まれます。これには、データの完全性の侵害やシステム ダウンタイムなどの直接的な影響と、評判の失墜や規制上の罰則などの間接的な影響の両方が含まれます。AI システムは規模と実行するタスクの重要性から、その影響はしばしば拡大します。例えば、医療診断に使用される AI システムへの攻撃が成功すると、誤診につながり、患者の健康に影響を与えるだけでなく、関係する組織に重大な法的、信頼、そして評判の損失をもたらす可能性があります。

### リスクの優先順位付け

発生可能性と影響度評価を組み合わせることで、リスクの優先順位付けの基礎となり、リスク対応の意思決定に役立ちます。一般的に、組織はリスク ヒートマップを使用して、リスクを影響度と発生可能性に基づいて視覚的に分類します。このアプローチは、リスク コミュニケーションと意思決定を促進します。これにより、経営陣は最も重大性の高い（発生可能性と影響度が高い）リスクに焦点を当てることができます。

## 3. リスク対応

リスク対応とは、リスクをどのように処理するか（移転、回避、受容、軽減）を決定することです。軽減には、管理策の選択と実装が含まれます。データ汚染、モデル窃盗、敵対的攻撃など、AI システムに関連する固有の脆弱性や脅威が存在するため、このプロセスは非常に重要です。効果的なリスク対応は、堅牢で信頼性の高い AI にとって不可欠です。

リスク対応の選択肢は以下のとおりです。

- **軽減**

	リスクの発生可能性または影響を低減するための管理策を実装します。これは、AI サイバー セキュリティ リスクを管理するための最も一般的なアプローチです。このリソースに記載されている多くの管理策と、以下の「管理策の選択」サブ セクションを参照してください。
	- 例: 悪意のあるデータをモデルに送り込み学習プロセスを破壊してパフォーマンスに悪影響を与えるデータ汚染攻撃を防止することを目的とした、データ検証プロセスの強化。

- **移転**

	リスクをサードパーティーに移転すること。通常は、転移学習、連合学習、保険、または特定の機能の外部委託を通じて行われます。
	- 例: AI モデルの学習、ホスティング、データ保存に堅牢なセキュリティ対策を備えたサードパーティーのクラウド サービスを利用することで、データ侵害やインフラ攻撃のリスクを移転させます。

- **回避**

	リスクを完全に排除するために計画や戦略を変更します。これには、リスクが高すぎると判断される分野では AI を使用しないことが含まれる場合があります。
	- 例: データ侵害のリスクを十分に軽減できない場合、機密性の高い個人データを処理するための AI システムの導入を見送ります。

- **受容**

	リスクを認識し、リスクを軽減するための具体的な措置を講じることなく、潜在的な損失を負うことを決定します。このオプションは、リスクへの対応コストが潜在的な影響を上回る場合に選択されます。
	- 例: 影響度が低いと考えられる、機密性が低いアプリケーションにおいて、モデル反転攻撃（攻撃者がモデル出力から公開されている入力データを再構築しようとする攻撃）の最小限のリスクを受け入れます。

## 4. リスク コミュニケーションと監視

リスク管理活動への意識向上と支援を確保するため、ステークホルダーとリスク情報を定期的に共有します。

このプロセスにおける中心的なツールとなるのがリスク登録簿です。これは、特定されたすべてのリスク、その属性（重大度、対応計画、責任者、ステータスなど）、そしてそれらを軽減するために実施された管理策を包括的に記録するリポジトリとして機能します。ほとんどの大規模組織では、既にこのようなリスク登録簿が整備されています。組織全体でリスクに関する効果的なコミュニケーションを促進するためには、AI リスクとエンタープライズ リスク管理で選択された用語を整合させることが重要です。

## 5. 責任の分担

選択した脅威ごとに、その対処責任者を特定します。デフォルトでは、AI システムを構築およびデプロイする組織が責任を負いますが、構築とデプロイは異なる組織が行う場合があり、モデルのホスティングやアプリケーション実行用のクラウド環境の提供など、構築とデプロイの一部は他の組織に委ねられることもあります。一部の側面は責任の共有となります。

AI システムの一部コンポーネントがホストされている場合、関連する脅威に対するすべての管理策に関する責任をホスティング プロバイダーと共有することになります。これは、責任マトリックスなどのツールを使用してプロバイダーと調整する必要があります。コンポーネントには、モデル、モデル拡張、アプリケーション、またはインフラストラクチャが含まれます。[モデルをそのまま使用する場合の脅威モデル](https://owaspai.org/docs/ai_security_overview/#threat-model-with-controls---genai-as-is)を参照してください。

外部の関係者が特定のリスクの軽減方法についてオープンでない場合は、その情報を要求することを検討してください。それでも不明な場合は、1) リスクを受容する、2) 独自の軽減策を提供する、3) サードパーティーと連携せずにリスクを回避する、のいずれかの選択肢があります。

## 6. 外部責任の検証

他の組織が責任を負う脅威については、当該組織が適切に対処しているかどうかの保証を得る必要があります。これには、これらの脅威に関連する管理策が含まれます。

例: サードパーティーのセキュリティ対策の定期的な監査と評価。

## 7. 管理策の選択

次に、使用事例に関連し、責任範囲に含まれる脅威について、関連する管理策（脅威（またはその親カテゴリ）の直下にリストされている管理策と、普遍的に適用される一般的な管理策の両方）をレビューします。各脅威のリスクを軽減する管理策の概要については、[基本要素表](https://owaspai.org/goto/periodictable/)を参照してください。各管理策について、その目的を考慮し、実装する価値があるかどうか、またどの程度実装する必要があるかを評価します。この決定は、実装コストと、管理策が脅威にどれだけ効果的に対処できるか、そして関連するリスクのレベルを比較検討する必要があります。これらの要因は、管理策を適用する順序にも影響を与えます。リスクが最も高い脅威から開始し、コストが低く、すぐに効果が出る管理策（「簡単に適用できるもの」）を優先します。

管理策には、多くの場合、品質関連のパラメーターがあり、特定の状況やリスク レベルに合わせて調整する必要があります。例えば、入力データに追加するノイズの量を決定したり、異常検出に適切なしきい値を設定したりすることが含まれます。シミュレーション環境でこれらの管理策の有効性をテストすることで、パフォーマンスとセキュリティへの影響を評価し、適切なバランスを見つけることができます。このチューニング プロセスは、シミュレーション テストと実際の運用環境からのフィードバックの両方から得られる知見を活用し、継続的に実施する必要があります。

十分な対策を講じたと言えるのはいつでしょうか？特定されたすべてのリスクが対処可能、つまり移転、回避、または受容可能になった時点で、AI システムは十分にセキュアであると判断されます。受容は、場合によっては初めに対応することなく直接行うことができますが、場合によっては、リスクを許容レベルに抑えるための管理策を講じる必要があります。

## 8. 残留リスクの受容

最終的には、実装した管理策を考慮して、各脅威に関して残っているリスクを受容する必要があります。

## 9. 選択した管理策のさらなる管理

継続的な監視、文書化、レポート、およびインシデント対応が必要です。詳細は、[SECPROGRAM（セキュリティ プログラム）](https://owaspai.org/goto/secprogram/)を参照してください。

## 10. 継続的なリスク評価

新たな脅威を検知し、対応するために継続的な監視を実施します。進化する脅威とインシデント対応活動からのフィードバックに基づいて、リスク管理戦略を更新します。

例: 新たな脆弱性に対応するため、リスク対応計画を定期的に見直し、更新します。
...
AI と著作権は、複雑でしばしば未解決の問題を提起する、数ある法と政策（公的および私的の両方）の領域のうちの 2 つです。AI の出力または生成されたコンテンツは、米国の著作権法によってまだ保護されていません。他の多くの法域では、このような素材の知的財産保護に関する正式な状況はまだ発表されていません。一方、入力コンテンツ、テキスト、学習データなどを提供する人間の貢献者は、そのような素材の著作権を所有する場合があります。最後に、AI の学習における特定の著作権で保護された素材の使用は、[フェア ユース](https://en.wikipedia.org/wiki/Fair_use)とみなされる可能性があります。

### AI と著作権のセキュリティ

AI 分野において、企業は、知的財産権、特に著作権に広範な影響を及ぼす可能性のある、数多くのセキュリティ上の脅威に直面しています。大規模データ学習モデルを含む AI システムが高度化するにつれ、著作権侵害の懸念が高まっています。これは、膨大な量のデータを処理する AI モデルの開発と学習の必要性が一因となっており、これらのデータには著作物が含まれる可能性があります。このような場合、著作物が所有者の許可なく、また AI モデルの運用者やプロバイダーの同意なしに学習データに挿入されると、著作権侵害による重大な財務リスクと風評リスクが生じ、データセット全体が破損する可能性があります。

AI を取り巻く法的課題は多面的です。一方では、AI モデルの学習に著作物を使用することが著作権侵害に該当するかどうかという問題があり、開発者が法的請求の対象となる可能性があります。他方では、業界の大部分は、AI によって生成された著作物の所有権と、学習データにおけるライセンス許諾されていないコンテンツの使用に取り組んでいます。この法的曖昧さは、開発者、コンテンツ作成者、著作権所有者など、すべての関係者に影響を及ぼします。

### AI と著作権に関する訴訟

最近の訴訟（執筆時点: 2024 年 4 月）は、これらの問題の緊急性を浮き彫りにしています。例えば、Stability AI、Midjourney、DeviantArt を相手取った集団訴訟では、ウェブ スクレイピングされた画像を用いてツールが学習することで、数百万人のアーティストの権利を侵害したと主張されています。

同様に、Getty Images が Stability AI に対し、自社のカタログから無断で画像を使用し、アート生成 AI の学習に使用したとして提訴したことは、著作権紛争がエスカレートする可能性を浮き彫りにしています。同様のシナリオを想像してみてください。サプライヤーが膨大な量の学習データを顧客システムに提供し、そのデータが、ライセンスや使用許可のない保護された作品、データセット、または素材のブロックによって侵害されているという状況です。

### AI 生成ソースコードの著作権 

ソースコードは、開発者の革新性と創造性を体現するものであり、ソフトウェア開発会社にとって重要な知的財産（IP）資産です。従って、ソースコードは、著作権、特許、営業秘密などを通じて知的財産保護の対象となります。ほとんどの場合、人間が生成したソースコードは、作成と同時に著作権が発生します。

しかし、人間の介入なしにソースコードを生成できる AI システムの出現は、知的財産制度に新たな課題を突きつけています。例えば、AI が生成したソースコードの作成者は誰なのか？誰がそのソースコードに対する知的財産権を主張できるのか？AI が生成したソースコードは、サードパーティーによってどのようにライセンス供与され、活用されるのか？

これらの問題は、現在の知的財産に関する法規制の枠組みが AI 生成作品の知的財産権を適切に規定していないため、容易に解決できるものではありません。さらに、AI が生成したソースコードは、既存のコードやデータソースから派生したものである場合もあり、完全に新規なものではない可能性があります。従って、AI によって生成されたソースコードの起源とプロセスを徹底的に分析し、その知的財産への影響を判断し、企業の知的財産資産を確実に保護することが不可欠です。このプロセスにおいては、知的財産およびテクノロジー分野を専門とする法律専門家に相談する必要があります。

例えば、GitHub、OpenAI、Microsoft に対し、ソースコードの著作権とライセンスに関する訴訟を提起した、現在も審理中の事例は、3 社が著作権を侵害したと主張する特定のコードの作成者によって提起された、ソースコードの著作権とライセンスに関する複雑な問題を示しています。詳細は、[こちら (GitHub Copilot copyright case narrowed but not neutered • The Register)](https://www.theregister.com/2024/01/12/github_copilot_copyright_case_narrowed/)を参照してください。

### 著作権損害賠償

AI ベンダーは、特定の状況下において、自社モデルの著作権問題への責任を負い始めている点に留意してください。Microsoft は、いわゆる [Copilot Copyright Commitment](https://www.microsoft.com/en-us/licensing/news/microsoft-copilot-copyright-commitment) をユーザーに提供しています。これは、Copilotが生成したコードの著作権に関する法的損害について、ユーザーを補償するものです。ただし、クライアントが Copilot のコンテンツ フィルターなどの安全システムを使用し、特定のサービスを利用していることなど、[いくつかの条件](https://learn.microsoft.com/en-us/legal/cognitive-services/openai/customer-copyright-commitment)が満たされる必要があります。Google Cloud は、[Generative AI indemnification（生成 AI に関する補償）](https://cloud.google.com/blog/products/ai-machine-learning/protecting-customers-with-generative-ai-indemnification)を提供しています。Microsoft の補償に関する詳細は、[The Verge の記事](https://www.theverge.com/2023/9/7/23863349/microsoft-ai-assume-responsibility-copyright-lawsuit)、および[補償要件に関する Microsoft の方針](https://www.directionsonmicrosoft.com/blog/why-microsofts-copilot-copyright-commitment-may-not-mean-much-for-customers-yet/)を参照してください。

### 生成 AI モデルは本当に既存の著作物をコピーするのか? 

生成 AI モデルは本当に著作権で保護されている可能性のある既存の作品を検索するのでしょうか？ 本質的には、いいえです。生成 AI モデルは、学習セットに含まれるコードや画像の例をすべて保存するのに十分な容量を持っていません。代わりに、学習中に、取得したデータから物事の仕組みに関するパターンを抽出し、その後、それらのパターンに基づいて新しいコンテンツを生成します。このコンテンツの一部には既存の作品の痕跡が見られる場合もありますが、それは偶然の一致です。本質的には、モデルはコード ブロックを正確に思い出すのではなく、コーディングの「理解」に基づいて新しいコードを作成します。人間と同様に、この理解によって、以前見たことのあるものの一部を再現できる場合がありますが、それは正確な記憶に基づいているため、必ずしもそうではありません。とはいえ、これは音楽業界でも見られる難しい議論です。ミュージシャンがコード シーケンスを思いついたのは、多くの曲からこの種類のシーケンスが機能することを学んだからであり、偶然にも既存のものを作成したのでしょうか、それとも既存の曲をそのままコピーしたのでしょうか？

### リスクの軽減

組織は、AI システムにおける著作権侵害リスクを軽減するための重要な戦略をいくつか採用しています。これらの戦略を早期に導入することで、AI システム運用の後の段階で修正するよりも、はるかに費用対効果の高いものとなります。それぞれに一定の財務コストと運用コストは発生しますが、「確実な節約」がプラスの結果をもたらす可能性があります。具体的には、以下のようなものが挙げられます。

- 特定の学習データの出力を軽減するための対策を講じます。OWASP AI Exchange は、[モデル出力によるデータ漏洩](https://owaspai.org/goto/disclosureuseoutput/)という脅威を例に挙げてこれをカバーしています。
- 包括的な知的財産監査: 徹底的な監査を実施することで、AI システム全体に関連するすべての知的財産を特定できます。これは必ずしもデータセットだけでなく、ソースコード、システム、アプリケーション、インターフェース、その他の技術スタック全体に適用されます。
- 明確な法的枠組みとポリシー: AI の利用に関する法的ポリシーと手順を策定し、それらを施行することで、著作権を含む最新の知的財産法に準拠していることを保証します。
- データ ソーシングにおける倫理: AI モデルの学習に使用するすべてのデータが社内で作成されたか、必要なすべての許可を得て取得されたか、組織の意図する用途に十分なライセンスが付与されたパブリック ドメインから取得されていることを確認し、倫理的にデータを取得します。
- AI 生成コンテンツの所有権の定義: AI システムによって生成されたコンテンツの所有権を明確に定義し、どのような条件下で使用、共有、配布できるかを含めます。
- 機密保持および営業秘密の手続き: 厳格な手続きは、営業秘密の地位を維持しながら、資料の機密性を保護するのに役立ちます。
- 従業員研修: 組織の AI 知的財産ポリシーの意義と重要性、そして知的財産侵害の可能性について従業員を研修することで、リスク回避能力を高めることができます。
- コンプライアンス監視システム: 最新の監視システムを適切に活用することで、AI システムによる潜在的な侵害のチェックに役立ちます。
- 知的財産侵害への対応計画: 積極的な計画は、潜在的な侵害請求に迅速かつ効果的に対応するのに役立ちます。
- 考慮すべきその他のリスク軽減要因としては、組織の使用目的だけでなく、AI システムによる将来のあらゆる使用に関して、AI サプライヤーからライセンスや保証を取得することなどが挙げられます。また、弁護士の助言を得ながら、潜在的な侵害請求に対応するために、サプライヤーに課すその他の契約上の拘束力のある義務についても検討する必要があります。
...
- [Using generative AI and protecting against copyright issues | World Economic Forum -weforum.org](https://www.weforum.org/agenda/2024/01/cracking-the-code-generative-ai-and-intellectual-property/)
- [Legal Challenges Against Generative AI: Key Takeaways | Bipartisan Policy Center](https://bipartisanpolicy.org/blog/legal-challenges-against-generative-ai-key-takeaways/)
...
- [Recent Trends in Generative Artificial Intelligence Litigation in the United States | HUB | K&L Gates - klgates.com](https://www.klgates.com/Recent-Trends-in-Generative-Artificial-Intelligence-Litigation-in-the-United-States-9-5-2023)
- [Generative AI could face its biggest legal tests in 2024 | Popular Science - popsci.com](https://www.popsci.com/technology/generative-ai-lawsuits/)
...
- [The current legal cases against generative AI are just the beginning | TechCrunch](https://techcrunch.com/2023/01/27/the-current-legal-cases-against-generative-ai-are-just-the-beginning/?guccounter=1)
- [(Un)fair Use? Copyrighted Works as AI Training Data — AI: The Washington Report | Mintz](https://www.mintz.com/insights-center/viewpoints/54731/2024-01-10-unfair-use-copyrighted-works-ai-training-data-ai)
- [Potential Supreme Court clash looms over copyright issues in generative AI training data | VentureBeat](https://venturebeat.com/ai/potential-supreme-court-clash-looms-over-copyright-issues-in-generative-ai-training-data/)
- [AI-Related Lawsuits: How The Stable Diffusion Case Could Set a Legal Precedent | Fieldfisher](https://www.fieldfisher.com/en/insights/ai-related-lawsuits-how-the-stable-diffusion-case)
