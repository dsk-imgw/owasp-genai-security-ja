...
- **堅牢性 (robustness)** とは、入力の予想または予期しない変動に対して正確性 (accuracy) を維持する能力のことです。セキュリティの範囲は、これらの変動が悪意のあるものであるかどうか（*敵対的堅牢性*）に関するもので、通常の変動（*一般化堅牢性*）に対して必要な対抗策とは異なる対抗策が必要になることがよくあります。正確性 (accurary) と同様に、セキュリティは、通常の変動に対する堅牢なモデルの作成自体には関与していません。例外は、一般化堅牢性または敵対的堅牢性が関係する場合です。これは安全性 (safety) とセキュリティの共通の関心事となるためです。どちらに当てはまるかは、具体的なケースによって異なります。
