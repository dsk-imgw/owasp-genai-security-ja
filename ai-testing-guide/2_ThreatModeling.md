...
- **解説**
	...
	ベクトル＆エンベディングの弱点は、改ざんされた、あるいは操作された埋め込みが言語モデルの検索または推論パイプラインに導入されたときに発生します。これらの埋め込みは、通常、文脈理解を強化するために RAG（検索拡張生成）システムで使用されますが、微妙に改ざんされることで意味や文脈の優先順位が歪められる可能性があります。攻撃者は、関連性スコアをシフトさせたり検索動作を変更したりする敵対的なベクトルを挿入する可能性があります。これにより、意味の混乱、偏った出力、さらには間接的なベクトル操作によるプロンプティングが発生する可能性があります。埋め込みが未検証または改ざんされたデータセット、あるいはサードパーティのベクトルストアから取得される場合、これらのリスクはさらに増大します。

- **脅威シナリオ**

	プラグイン コンポーネント (SAIF #5) は、外部ベクトル データベース (SAIF #6) を照会するか、未検証のデータ ソース (18) から事前学習済みのエンベディングを読み込みます。これらのエンベディングは、巧妙に細工されたベクトル表現で汚染されており、推論中にモデルの利用 (9) 段階に送信されます。モデルは、エンベディングの歪みにより、ユーザーの元のプロンプトの意図や意味を誤って解釈し、無関係、有害、または操作されたコンテンツを表示する可能性があります。場合によっては、ベクトルがコンテキスト構築やスコアリングに与える影響に応じて、敵対的な埋め込みが有害な応答を増幅したり、整合（アライメント）制約を無視したりするために使用される可能性があります。
