...
- **解説**
	...
	ベクトル＆エンベディングの弱点は、改ざんされた、あるいは操作された埋め込みが言語モデルの検索または推論パイプラインに導入されたときに発生します。これらの埋め込みは、通常、文脈理解を強化するためにRAG（検索拡張生成）システムで使用されますが、微妙に改ざんされることで意味や文脈の優先順位が歪められる可能性があります。攻撃者は、関連性スコアをシフトさせたり検索動作を変更したりする敵対的なベクトルを挿入する可能性があります。これにより、意味の混乱、偏った出力、さらには間接的なベクトル操作によるプロンプティングが発生する可能性があります。埋め込みが未検証または改ざんされたデータセット、あるいはサードパーティのベクトルストアから取得される場合、これらのリスクはさらに増大します。
