...
表 1.1 では、SAIF に記載されている AI リスクを示しています。各リスク カテゴリとその説明、評価されたビジネスへの影響、発生可能性と影響に基づく対応するリスク レベル、および SAIF で特徴付けられるリスク所有者とともに示しています。モデル作成者は「自身または他者が使用するために AI モデルを学習または開発する者」、モデル消費者は「AI モデルを使用して AI を活用した製品やアプリケーションを構築する者」と定義されます。適切なリスク所有者は、管理策が適用される場所に基づきます (つまり、アプリケーション/モデル = モデル ユーザー、データ/インフラストラクチャ = モデル作成者、両方 = モデル作成者、モデル ユーザー)。
...
![AISystemArchitecture.png](:/acebc66b2a9d4957ac64627653c6f89b)

図 2: OWASP 脅威（LLM T10およびAI Exchange）の脅威モデルの SAIF ベースライン アーキテクチャの影響を受けるAI コンポーネントへのマッピング
</div>
