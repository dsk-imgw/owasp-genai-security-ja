...
**概要:** ユーザーの入力はシステムの玄関口であり、下流のすべてのコンポーネントはこれに依存しています。強力な入力検証、フィルタリング、制限がなければ、プロンプト インジェクション、データ漏洩、DoS、そして有害な感染の主な媒介物となります。

##### 直接プロンプトインジェクション (T01-DPIJ) と間接プロンプトインジェクション (T01-IPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)、[CWE-707](https://cwe.mitre.org/data/definitions/707.html)

**根拠:** 悪意を持って作成された入力（ユーザー プロンプトまたは埋め込まれた指示）は、指示を上書きしたり、推論チェーンを変更したり、接続ツールで意図しないアクションを引き起こしたりする可能性があります。

**推奨事項:**
- コンテンツをモデルに渡す前に、厳格な入力検証と正規化を適用します。
- プロンプト分離またはサンドボックス化（ユーザーとシステムの指示を分離）を使用します。
- ホワイト リスト ベースの指示および関数パターンを適用します。
- 敵対的プロンプト ファジングとレッド チーム テストを実施します。

##### サービス拒否 – モデル (T01-DoSM)

**マッピングされた CWE:** [CWE-400](https://cwe.mitre.org/data/definitions/400.html)、[CWE-770](https://cwe.mitre.org/data/definitions/770.html)、[CWE-787](https://cwe.mitre.org/data/definitions/787.html)

**根拠:** サイズが大きすぎる、不正な形式である、または敵対的な入力は、トークン化、GPU、またはコンピューティング能力を枯渇させ、パフォーマンスの低下やサービスの利用不能につながる可能性があります。

**推奨事項:**
- 入力の最大サイズとトークン制限を適用します。
- API ゲートウェイにレート制限とユーザーごとのクォータを適用します。
- サーキット ブレーカーとオート スケーリングを使用して、負荷の急増を緩和します。

##### 入力によってトリガーされる安全でない出力処理 (T01-IOH)

**マッピングされた CWE:** [CWE-116](https://cwe.mitre.org/data/definitions/116.html)、[CWE-79](https://cwe.mitre.org/data/definitions/79.html)

**根拠:** 悪意のある入力がレンダリングされた出力（HTML、Markdown、JSON など）に伝播し、インジェクション攻撃やクロスサイト スクリプティング攻撃を可能にする可能性があります。

**推奨事項:**
- すべてのレンダリングされた出力を無害化し、コンテキストに応じてエンコードします。
- データと制御文字を分離し、安全なテンプレートおよびレンダリング フレームワークを使用します。
- 表示前に厳格なコンテンツ タイプ検証を実施します。

##### モデルの毒性 / 信頼できない出力 (T01-MTU)

**マッピングされた CWE:** [CWE-707](https://cwe.mitre.org/data/definitions/707.html)、[CWE-345](https://cwe.mitre.org/data/definitions/345.html)、[CWE-1204](https://cwe.mitre.org/data/definitions/1204.html)

**根拠:** 巧妙に細工された、あるいは挑発的なユーザー入力は、モデルの動作にバイアスを与え、有害、差別的、あるいは根拠のない応答へと誘導する可能性があります。

**推奨事項:**
- ユーザー プロンプトを事前にスクリーニングするために、毒性分類器とバイアス分類器を統合します。
- 受信したリクエストにコンテキスト フィルターと感情フィルターを使用します。
- 高リスクまたはポリシー違反のケースは、人間によるレビュー ワークフローにエスカレーションします。

#### (3) ユーザーへの出力

**概要:** ユーザー/接続システムへのラスト マイル。制御されていない場合、過剰な主体性、プロンプト情報の漏洩、誤情報、そしてセキュアでないレンダリングの原因となります。

##### 過剰な主体性 (T01-EA)

**対応CWE:** [CWE-284](https://cwe.mitre.org/data/definitions/284.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)

**根拠:** アクションを伴うモデル出力（生成されたコマンド、API 呼び出し、ワークフロー トリガーなど）は、権限やユーザーの監視なしに、特権操作や不可逆操作を実行できます。

**推奨事項:**
- すべてのアクション可能な出力に対して、最小限の権限スコープを適用します。
- UI 駆動型アクションをレンダリングまたは実行する前に、ポリシーと承認チェックを適用します。
- 許可リストを維持し、影響度の高い操作や機密性の高い操作については、明示的な人間による承認を求めます。

##### 機密性の高いプロンプトの漏洩 (T01-SPL)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-209](https://cwe.mitre.org/data/definitions/209.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-532](https://cwe.mitre.org/data/definitions/532.html)

**根拠:** モデルの出力、エラー メッセージ、またはログによって、会話コンテキストに埋め込まれた非表示のプロンプト、資格情報、API 鍵、または個人情報が意図せず明らかになる可能性があります。

**推奨事項:**
- レンダリングまたはログ出力前に、機密情報、個人識別情報（PII）、およびシステム命令をｈ秘匿化します。
- 構造化されたエラー ラッパーを使用します。生のスタック トレースやバックエンド エラーは公開してはなりません。
- ユーザーに表示されるログとオペレータ ログを分離し、プロンプトや機密情報の漏洩を防ぐため、DLP スキャンを適用します。

##### 誤情報 (T01-MIS)

**マッピングされた CWE:** [CWE-345](https://cwe.mitre.org/data/definitions/345.html)、[CWE-1204](https://cwe.mitre.org/data/definitions/1204.html)

**根拠:** 根拠のない、捏造された、またはバイアスのある記述は、UI に表示されると信憑性があるように見せかけ、ユーザーの信頼を損なったり、虚偽の情報を拡散したりする可能性があります。

**推奨事項:**
- 高リスクまたは事実に基づく主張については、根拠と引用元の確認を義務付けます。
- 不確実な応答に対して、検証信頼度スコアと「レビューが必要」フラグを統合します。
- フラグが付けられた出力を、人間によるレビューまたはモデレーション パイプラインにルーティングします。

##### セキュアでない出力処理 (T01-IOH)

**マッピングされた CWE:** [CWE-116](https://cwe.mitre.org/data/definitions/116.html)、[CWE-79](https://cwe.mitre.org/data/definitions/79.html)、[CWE-75](https://cwe.mitre.org/data/definitions/75.html)

**根拠:** リッチ テキスト、HTML、または Markdown 形式でレンダリングされた、無害化されていないモデル出力は、下流のクライアントでスクリプト実行、インジェクション、または UI 操作につながる可能性があります。

**推奨事項:**
- 構造化形式（JSON、平文など）からの出力は、コンテキスト認識エンコーディングを使用してレンダリングします。
- 許可リストに登録された要素と属性を使用して、HTML/Markdown を無害化します。
- すべてのレンダリング環境で、安全でない埋め込み、リンク、インライン スクリプトを無効にします。

#### (4) アプリケーション

**概要:** セッション、API、およびビジネス ロジックを管理するオーケストレーションの頭脳です。この層における検証、エラー処理、またはアクセス制御の脆弱性は、アプリケーション スタック全体にわたるシステム全体のセキュリティ侵害につながる可能性があります。

##### プロンプト インジェクション (T01-DPIJ、T01-IPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)

**根拠:** 検証されていない、またはエスケープされていない入力がモデル オーケストレーション ロジックまたはプロンプト テンプレートに挿入されると、指示が上書きされたり、ビジネス ルールが迂回されたり、意図しないシステム アクションがトリガーされたりする可能性があります。

**推奨事項:**
- すべての入力に対して、厳密なスキーマ検証と正規化を実行します。
- ユーザーが作成する指示、開発者の指示、システムの指示の役割を分離します。
- ユーザーの入力とモデル オーケストレーションの間に、安全なインタープリターまたは仲介レイヤーを導入します。
- QA の一環として、敵対的プロンプト インジェクション テストを実施します。

##### 機密情報の漏洩 (T01-SID、T01-SPL)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-209](https://cwe.mitre.org/data/definitions/209.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**根拠:** ログ、プロンプトのコンテキスト、またはプラグインのレスポンスを通じて、シークレット、認証情報、または内部構成の詳細が漏洩し、機密データやビジネス ロジックが露出する可能性があります。

**推奨事項:**
- ログ、プロンプト、および API レスポンスからシークレットと個人識別情報（PII）を削除します。
- 機密構成データへの RBAC とスコープ指定アクセスを適用します。
- スタック トレースと内部状態を隠蔽する、安全でユーザー フレンドリーなエラー処理を実装します。
- ログとテレメトリに DLP スキャンを適用します。

##### サービス拒否 – モデル (T01-DoSM)

**マッピングされた CWE:** [CWE-400](https://cwe.mitre.org/data/definitions/400.html)、[CWE-770](https://cwe.mitre.org/data/definitions/770.html)、[CWE-787](https://cwe.mitre.org/data/definitions/787.html)

**根拠:** オーケストレーション サービスまたは推論サービスへの過剰なリクエストや不正なリクエストは、コンピューティング、メモリ、またはトークン リソースを飽和させ、サービスが利用できなくなる可能性があります。

**推奨事項:**
- API ゲートウェイとオーケストレーション層にレート制限とサーキット ブレーカーを適用します。
- 入力サイズ、トークン、およびフォーマットの検証を強制します。
- テナント、API、またはモデル インスタンスごとにワークロードの分離とクォータを実装します。
- 異常な消費パターンを検出するために、ランタイム指標を監視します。

##### モデルの毒性 / 誤情報 (T01-MTU、T01-MIS)

**マッピングされた CWE:** [CWE-345](https://cwe.mitre.org/data/definitions/345.html)、[CWE-1204](https://cwe.mitre.org/data/definitions/1204.html)

**根拠:** オーケストレーションにグラウンディング、信頼度しきい値、またはモデレーション層がない場合、アプリケーションに組み込まれたモデルは、有害なコンテンツ、バイアスのあるコンテンツ、または虚偽のコンテンツを生成する可能性があります。

**推奨事項:**
- 信頼できるデータソースを使用して、グラウンディングと事実の整合性チェックを実装します。
- 推論パイプラインに毒性フィルターとバイアス フィルターを統合します。
- 信頼性の低い、またはリスクの高い出力は、配布前にレビュー対象としてフラグ付けします。
- モデルの信頼性と公平性の指標を継続的に評価します。

##### セキュアでない出力処理 (T01-IOH)

**マッピングされた CWE:** [CWE-79](https://cwe.mitre.org/data/definitions/79.html)、[CWE-116](https://cwe.mitre.org/data/definitions/116.html)、[CWE-75](https://cwe.mitre.org/data/definitions/75.html)

**根拠:** ダッシュボードや下流のクライアントにレンダリングされるモデル出力（HTML、Markdown、または JSON）が適切に無害化またはエンコードされていない場合、インジェクション、クロスサイト スクリプティング、またはデータ破損につながる可能性があります。

**推奨事項:**
- レンダリング前にコンテキスト エンコーディングとサニタイズを適用します。
- 安全でない HTML/Markdown タグおよび属性を削除またはエスケープします。
- 安全なテンプレート ライブラリまたはレンダリング フレームワークを使用します。
- サービス間の出力検証とコンテンツ タイプの境界を強化します。

##### 過剰な主体性 (T01-EA)

**マッピングされた CWE:** [CWE-284](https://cwe.mitre.org/data/definitions/284.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)

**根拠:** 自律エージェントまたはモデル駆動型 API は、適切な監視や承認なしに、トランザクションの開始やファイルの変更などの特権アクションを実行する可能性があります。

**推奨事項:**
- モデル プラグイン、エージェント、および統合に対して、最小限の権限アクセスを適用します。
- 機密性の高い操作と外部サービス呼び出し用の許可リストを維持します。
- 影響の大きいアクションについては、二次承認または人間による検証を必須とします。
- エージェントが開始したすべての操作をログに記録し、監査を実施して説明責任を果たします。

#### (5) エージェント/プラグイン

**概要:** システムの拡張機能であり、IPIJ、シークレットの取り扱い、改ざん、過剰なアクション、および安全でないワークフローに対して脆弱です。

##### 間接プロンプト インジェクション (T01-IPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)

**根拠:** プラグインまたは接続ツールは、ユーザー プロンプトまたはシステム プロンプト内に埋め込まれた細工された命令や隠された命令を受け取る可能性があり、これらの命令によって下流のコンポーネントが操作されたり、意図された動作が変更されたり、安全でないコードの実行が引き起こされたりする可能性があります。

**推奨事項:** 厳格な入出力スキーマを適用し、すべてのパラメータをエスケープまたは無害化します。モデル生成コンテンツからの動的コード評価や直接コマンド実行を禁止します。

##### 機密情報の開示 (T01-SID)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**根拠:** モデル、プラグイン、または接続サービスにおいて、データ保護またはコンテキスト認識が不十分なため、ログ、プロンプト、または API レスポンスを通じて、認証情報、トークン、個人情報などの機密データが漏洩します。

**推奨事項:** スコープが限定された、有効期間の短い認証情報を使用します。ツールおよびモデルの出力における機密フィールドを秘匿化します。データ最小化と必要最小限のアクセス制御を適用します。

##### モデルの改ざん/漏洩 (T01-MTD)

**マッピングされた CWE:** [CWE-276](https://cwe.mitre.org/data/definitions/276.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)、[CWE-494](https://cwe.mitre.org/data/definitions/494.html)

**根拠:** モデル アーティファクト、重み、または構成は、ファイル権限の脆弱性、完全性チェックの欠如、または安全でないデプロイメント パイプラインによって変更、置換、または流出される可能性があり、攻撃者によるモデルの動作の変更や知的財産の漏洩を許します。

**推奨事項:** ファイルおよびストレージの権限を強化し、署名付きマニフェストを介してモデルの整合性を検証し、デプロイメント前にすべてのモデル アーティファクトにデジタル署名と検証を義務付けます。

##### 過剰な主体性 (T01-EA)

**マッピングされた CWE:** [CWE-284](https://cwe.mitre.org/data/definitions/284.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)

**理由:** アクセス制御が不十分であるか、権限の委譲が制限されていないために、モデルまたは自律エージェントが、特権 API の呼び出し、外部システムの変更、承認されていないトランザクションの実行など、本来の権限を超えたアクションを実行します。

**推奨事項:** アクションごとに最小権限を適用します。機密性の高い操作にはポリシー ゲートを実装します。高リスクまたは取り返しのつかないアクションには、人間による承認を求めます。

##### 脆弱な外部ワークフロー (T01-VEW)

**マッピングされた CWE:** [CWE-829](https://cwe.mitre.org/data/definitions/829.html)、[CWE-918](https://cwe.mitre.org/data/definitions/918.html)、[CWE-502](https://cwe.mitre.org/data/definitions/502.html)

**根拠:** モデル統合ツールまたは外部ワークフロー コンポーネントは、信頼できない依存関係、SSRF ベクトル、または安全でないデシリアライゼーションによって悪用される可能性があり、攻撃者は内部ネットワークに侵入し、データを盗み出したり、任意のコードを実行したりする可能性があります。

**推奨事項:** 厳格なツールのホワイトリストと出力プロキシ制御を維持し、外部レスポンスのコンテンツ タイプとスキーマの検証を実施します。

#### (6) 外部ソース

**概要:** 外部世界との橋渡しとなるため、検証されていないデータは有害な情報を注入したり、安全でないアクションを引き起こしたり、誤情報を拡散したりする可能性があります。

##### 間接プロンプト インジェクション (T01-IPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)

**根拠:** プラグインまたは取得コンポーネントは、外部ソース（Web ページ、文書、API）からの細工されたコンテンツや悪意のあるコンテンツを処理する可能性があり、プロンプト操作によって隠された命令を挿入したり、モデルの挙動を変更したりする可能性があります。

**推奨事項:** 取得したすべての外部コンテンツを無害化および正規化し、受け入れ可能なコンテンツの種類と形式を制限します。取得したデータを分離し、ラベル付けすることで、コンテキスト間のプロンプト インジェクションを防止します。

##### モデルの改ざん/漏洩 (T01-MTD)

**マッピングされた CWE:** [CWE-276](https://cwe.mitre.org/data/definitions/276.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)、[CWE-494](https://cwe.mitre.org/data/definitions/494.html)

**根拠:** モデル ファイル、重み、または設定は、弱いストレージ権限、検証されていない更新、または安全でないパイプラインを通じて変更または漏洩される可能性があり、攻撃者による出力の改ざん、バックドアの挿入、または機密データの窃取が可能になります。

**推奨事項:** すべてのモデル アーティファクトに対して完全性と署名の検証を実装し、最小権限アクセスと明示的な変更承認を実施します。学習環境とデプロイメント環境全体にわたって、強化されたストレージ権限を適用します。

##### 機密情報の開示 (T01-SID)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**根拠:** AI モデルまたは接続ツールは、不十分な秘匿化またはアクセス制御により、ログ、レスポンス、または保存されたコンテキストを通じて機密データ（トークン、資格情報、個人識別子など）を公開する可能性があります。

**推奨事項:** ログと出力内の機密フィールドをマスクします。最小限の権限でスコープ指定された OAuth 認証情報を使用します。プロンプトとレスポンスのデータ フローにデータ損失防止（DLP）ポリシーを適用します。

##### 過剰な主体性 (T01-EA)

**マッピングされた CWE:** [CWE-284](https://cwe.mitre.org/data/definitions/284.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)

**理由:** モデルまたはエージェントが、適切な承認やコンテキスト ポリシーの検証なしに、特権が必要なアクションや意図しないアクション（機密性の高い API の呼び出し、リソースの変更、外部ツールの呼び出しなど）を自律的に実行します。

**推奨事項:** データ ソースとアクションに対して RBAC と許可リストを適用し、モデルによって開始される操作を実行する前にポリシーと安全性のチェックを実施し、サンドボックス化されたコネクタまたは分離されたコネクタを使用して外部アクセスを制限します。

##### 脆弱な外部ワークフロー (T01-VEW)

**マッピングされた CWE:** [CWE-829](https://cwe.mitre.org/data/definitions/829.html)、[CWE-918](https://cwe.mitre.org/data/definitions/918.html)、[CWE-502](https://cwe.mitre.org/data/definitions/502.html)

**根拠:** 外部ワークフローと連携する統合機能やツールは、信頼できない依存関係、SSRF、または安全でないデシリアライゼーションによって侵害され、不正なネットワーク アクセスやリモート コード実行につながる可能性があります。

**推奨事項:** 送信接続には出力プロキシと厳格なホワイト リストを適用し、安全なコンテンツ タイプを検証・適用し、署名付きリリースと SBOM 検証を通じてソフトウェア サプライチェーンの完全性を検証します。

##### データ/モデルの汚染 (T01-DMP)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-494](https://cwe.mitre.org/data/definitions/494.html)、[CWE-353](https://cwe.mitre.org/data/definitions/353.html)

**根拠:** 攻撃者は、パイプラインの学習、微調整、または更新中に悪意のあるデータを挿入したり、モデル アーティファクトを操作したりすることで、バイアスのある出力、バックドア、またはパフォーマンスの低下を引き起こします。

**推奨事項:** データの来歴とレピュテーションのスコアリング機構を確立し、敵対的サンプルと異常テストを実施し、パイプライン全体のデータセットとモデル アーティファクトに暗号完全性チェックを適用します。

#### (7) 入力の処理

**概要:** フィルター層。解析/スキーマの適用が弱いため、攻撃者の入力/インジェクションがすり抜けてしまう可能性があります。

##### プロンプト インジェクション (T01-DPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)

**根拠:** 悪意のあるユーザーまたはシステム入力がモデルのプロンプトを操作し、指示を上書きしたり、新しいゴールを挿入したり、下流のツールや接続されたシステムで意図しないアクションを引き起こしたりする可能性があります。

**推奨事項:** 厳格な入力スキーマと強力な型付けを適用し、安全でない制御シーケンスとエスケープ文字を削除し、プロンプトのアセンブリ前にユーザー入力をサンドボックス化して分離します。

##### 敵対的入力回避 (T01-AIE)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-1384](https://cwe.mitre.org/data/definitions/1384.html)

**根拠:** 攻撃者は、モデルの検出や分類の境界を回避するために、敵対的な入力（例: 改変されたトークン、Unicodeトリック、難読化されたペイロード）を作成し、予測ミスや安全フィルターの迂回を引き起こします。

**推奨事項:** Unicode およびエンコーディングのバリエーションを正規化および無害化し、敵対的堅牢性テストを実施し、階層化された入力検証と信頼度閾値を適用します。

##### 機密情報の開示 (T01-SID、T01-LSID、T01-SPL)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**理由:** 機密データ（API 鍵、シークレット、PII、学習データなど）は、入力データの秘匿化、冗長エラー、または安全でないコンテキストの保持により、取り込み、推論、またはログ記録中に漏洩します。

**推奨事項:** 機密性の高い用語に対して取り込み時の秘匿化を適用します。ログ内のシークレットをマスクまたはトークン化します。データ漏洩を防ぐため、ログ、エラー トレース、ツールのレスポンスを無害化します。

##### サービス拒否 – モデル (T01-DoSM)

**マッピングされた CWE:** [CWE-400](https://cwe.mitre.org/data/definitions/400.html)、[CWE-770](https://cwe.mitre.org/data/definitions/770.html)、[CWE-787](https://cwe.mitre.org/data/definitions/787.html)

**根拠:** 入力サイズが大きすぎる、または形式が不正な場合や、リクエスト レートが無制限の場合、モデル推論サービスの GPU、メモリ、または CPU リソースが枯渇し、パフォーマンスの低下やサービスの停止につながる可能性があります。

**推奨事項:** 入力サイズとレートのクォータを適用し、推論実行前にバッファ サイズとテンソル構造を検証します。

##### 脆弱な外部ワークフロー (T01-VEW)

**マッピングされた CWE:** [CWE-829](https://cwe.mitre.org/data/definitions/829.html)、[CWE-918](https://cwe.mitre.org/data/definitions/918.html)、[CWE-502](https://cwe.mitre.org/data/definitions/502.html)

**根拠:** 外部ツールチェーン、Webhook、または取得フローは、信頼できない依存関係、SSRF、または安全でないデシリアライゼーションを通じて悪用され、内部ネットワークにアクセスされたり、任意のコードが実行されたりする可能性があります。

**推奨事項:** ドメイン ベースのホワイト リストとアウトバウンド プロキシの適用を使用し、取得したすべてのリソースと外部リソースに対して安全なコンテンツ タイプを検証して適用します。

#### (8) 出力の処理

**概要:** 配信前の安全ゲート。ここでの不具合により、機密データ、誤情報、安全でないコンテンツが漏洩する可能性があります。

##### ログ/ストレージ情報の開示 (T01-LSID)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-532](https://cwe.mitre.org/data/definitions/532.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**根拠:** ログまたは永続ストレージには、機密情報を含む生のモデル出力、ユーザー プロンプト、またはトークンが記録される可能性があります。秘匿化、暗号化、またはアクセス制御がなければ、これらの記録から機密情報、個人情報、または独自のコンテキストが漏洩する可能性があります。

**推奨事項:** 保存されたログと出力から機密コンテキストを削除します。ログ アクセスには RBAC と最小権限を適用します。無害化された汎用的なエラー メッセージを使用します。

##### 機密情報の開示 (T01-SID、T01-SPL、T01-TDL)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-522](https://cwe.mitre.org/data/definitions/522.html)

**根拠:** 出力層は、生成されたレスポンス、サマリー、または再呼び出しされたサンプルを通じて、機密情報、個人情報、または機密性の高い学習データを意図せず漏洩する可能性があります。

**推奨事項:** 出力後に DLP スキャンを適用します。クライアントに返す前に機密フィールドを暗号化またはマスクします。機密性の高い学習データ行の再呼び出しや逐語的な漏洩を防止します。

##### サービス拒否 – モデル (T01-DoSM)

**マッピングされた CWE:** [CWE-400](https://cwe.mitre.org/data/definitions/400.html)、[CWE-770](https://cwe.mitre.org/data/definitions/770.html)、[CWE-787](https://cwe.mitre.org/data/definitions/787.html)

**根拠:** 過度に大きい、または不正な形式の出力（例: テキスト生成の暴走、長い JSON シーケンス）は、下流のバッファをオーバーフローさせたり、レンダリング リソースを大量に消費したりして、可用性に影響を与える可能性があります。

**推奨事項:** 出力サイズとトークン制限に上限を設けます。サイズが大きすぎるレスポンスは隔離または切り捨てます。下流のバッファとレンダリング能力を検証します。

##### セキュアでない出力処理 (T01-IOH)

**マッピングされた CWE:** [CWE-79](https://cwe.mitre.org/data/definitions/79.html)、[CWE-116](https://cwe.mitre.org/data/definitions/116.html)、[CWE-75](https://cwe.mitre.org/data/definitions/75.html)

**根拠:** 信頼できないモデル出力が適切なエンコードなしでHTML、Markdown、またはコードとしてレンダリングされると、クライアントまたは下流のシステムでインジェクション攻撃やコンテンツ操作につながる可能性があります。

**推奨事項:** コンテキスト出力エンコーディングとホワイト リストに登録された無害化ルーチンを使用します。信頼できないテキストまたはコード ブロックに対してリッチ レンダリングを無効にします。厳格なコンテンツ タイプ境界を適用します。

##### 学習データの漏洩 (T01-TDL)

**マッピングされた CWE:** [CWE-201](https://cwe.mitre.org/data/definitions/201.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)

**根拠:** モデルは、学習データから、個人を特定できる情報や機密情報を含む、逐語的なスニペットや記憶されたコンテンツを出力する可能性があります。

**推奨事項:** 学習中に差分プライバシーを採用し、逐語的およびエントロピー ベースの漏洩フィルターを使用し、プロンプトと出力のログを秘匿化し、モデルのテレメトリまたはトレース データへのアクセスを制限します。

##### モデルの毒性／誤情報 (T01-MTU、T01-MIS)

**マッピングされた CWE:** [CWE-345](https://cwe.mitre.org/data/definitions/345.html)、[CWE-1204](https://cwe.mitre.org/data/definitions/1204.html)

**根拠:** フィルタリングされていないモデルの動作、または検証済みの情報源への十分な根拠付けがないため、生成された出力には有害な情報、バイアスのある情報、または誤情報が含まれる可能性があります。

**推奨事項:** 毒性およびバイアス フィルターを統合し、信頼できるデータセットへの根拠付けと引用を必須とし、信頼性が低い場合やバイアスが検出された場合はフォール バック対応を実装します。

##### 過剰な主体性 (T01-EA)

**マッピングされた CWE:** [CWE-284](https://cwe.mitre.org/data/definitions/284.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)

**根拠:** モデルまたはそれに接続されたツールは、明示的な承認や確認なしに、アクション（API 呼び出し、ファイル書き込み、システム変更など）を自動的に実行します。

**推奨事項:** アクションを許可リストに登録されたコマンドに制限し、実行前に承認とポリシー チェックを適用し、影響の大きい操作については明示的な人間による確認を求めます。

#### (9) モデル

**概要:** 中核のインテリジェンス。インジェクション、汚染、盗難、反転、DoS、および安全でない出力の標的となります。

##### プロンプト インジェクション (T01-DPIJ、T01-IPIJ)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-74](https://cwe.mitre.org/data/definitions/74.html)、[CWE-94](https://cwe.mitre.org/data/definitions/94.html)

**根拠:** 細工されたユーザーまたはシステム入力によって、モデルのプロンプト内の指示が上書き、操作、または挿入される可能性があり、モデルの意図した推論パスが変更されたり、信頼できないアクションが実行されたりする可能性があります。

**推奨事項:** システム、開発者、およびユーザーのプロンプトを独立したコンテキストに分離します。トークナイザー段階のフィルタリングと正規化を適用し、プロンプト操作に対する防御を強化するための敵対的学習を実施します。

##### サプライチェーン / データと微調整の汚染 (T01-SCMP、T01-DPFT、T01-RMP、T01-DMP)

**マッピングされた CWE:** [CWE-494](https://cwe.mitre.org/data/definitions/494.html)、[CWE-353](https://cwe.mitre.org/data/definitions/353.html)、[CWE-829](https://cwe.mitre.org/data/definitions/829.html)

**根拠:** モデルの学習または微調整に使用するデータ、依存関係、または重みは、侵害されたデータセット、悪意のあるモデル チェックポイント、またはサプライチェーン内の改ざんされたパッケージによって汚染または置換される可能性があります。

**推奨事項:** デジタル署名されたモデルの重みとデータセットを使用します。来歴とレピュテーション スコアリングを適用します。敵対的パターンの微調整データを無害化します。すべてのモデル コンポーネントでSBOMを維持します。

##### 敵対的入力回避 (T01-AIE)

**マッピングされた CWE:** [CWE-20](https://cwe.mitre.org/data/definitions/20.html)、[CWE-1384](https://cwe.mitre.org/data/definitions/1384.html)

**根拠:** 敵対的に擾乱された入力は、モデルの脆弱性を悪用して検出を回避したり、誤分類を引き起こしたりします。これは多くの場合、トークン レベルまたはエンベディング空間における微妙な操作によって行われます。

**推奨事項:** トークン化の前に入力を正規化します。データセット全体で堅牢性テストと敵対的テストを実施します。埋め込み分布のドリフトや異常を監視します。

##### 機密情報の開示 / 学習データの漏洩 (T01-SID、T01-TDL、T01-LSID、T01-SPL)

**マッピングされた CWE:** [CWE-200](https://cwe.mitre.org/data/definitions/200.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)、[CWE-201](https://cwe.mitre.org/data/definitions/201.html)

**根拠:** モデルのパラメータまたは出力は、フィルタリングされていない応答やモデルの反転試行を通じて、記憶された学習データ、機密性の高いコンテキスト、またはプライベート属性を公開する可能性があります。

**推奨事項:** 学習中に差分プライバシー（例: DP-SGD）を適用します、逐語的シーケンス再呼び出しをブロックし、システム プロンプト内の機密トークンを削除し、推論時のログ記録を制限または無害化します。

##### モデル反転 / メンバーシップ推論 (T01-MIMI)

**マッピングされた CWE:** [CWE-203](https://cwe.mitre.org/data/definitions/203.html)、[CWE-359](https://cwe.mitre.org/data/definitions/359.html)

**根拠:** 攻撃者は、モデルにクエリを実行し、特定のデータ レコードが学習中に使用されたかどうかを推測したり、反転手法を用いて機密性の高い学習例を再構築したりします。

**推奨事項:** DP-SGD またはその他のノイズ ベースのプライバシー機構を使用します。レート制限と出力のランダム化を適用します。メンバーシップ推論に特化したレッド チーム演習を実施し、耐性を検証します。

##### サービス拒否 – モデル (T01-DoSM)

**マッピングされた CWE:** [CWE-400](https://cwe.mitre.org/data/definitions/400.html)、[CWE-770](https://cwe.mitre.org/data/definitions/770.html)、[CWE-787](https://cwe.mitre.org/data/definitions/787.html)

**根拠:** モデル コンテキストの長さが長すぎる、プロンプト チェーンが複雑である、または推論ペイロードの形式が不正な場合、GPU/CPUリソースに過負荷がかかり、パフォーマンスの低下やサービス停止につながる可能性があります。

**推奨事項:** モデル コンテキストとトークンの制限を制限し、異常な推論パターンや異常を検出し、サービスバッファを強化し、リクエストごとにリソース割り当てを適用します。

##### セキュアでない出力処理 / 安全でない統合 (T01-IOH、T01-VEW)

**マッピングされた CWE:** [CWE-79](https://cwe.mitre.org/data/definitions/79.html)、[CWE-116](https://cwe.mitre.org/data/definitions/116.html)、[CWE-829](https://cwe.mitre.org/data/definitions/829.html)

**根拠:** モデル出力には、信頼できないデータや外部システムに渡される安全でないフォーマットが含まれている可能性があります。また、統合によって出力が無害化されずに処理される可能性があり、インジェクションやワークフローの侵害につながる可能性があります。

**推奨事項:** すべてのモデル出力を無害化およびエンコードします。統合はホワイトリストに登録されたツールおよび信頼できるドメインに限定します。モデルとツールの実行の間にポリシーと検証レイヤーを適用します。

##### モデルの盗難/持ち出し (T01-MTR、T01-MTD)

**マッピングされたCWE:** [CWE-276](https://cwe.mitre.org/data/definitions/276.html)、[CWE-285](https://cwe.mitre.org/data/definitions/285.html)、[CWE-494](https://cwe.mitre.org/data/definitions/494.html)

**根拠:** モデル アーティファクト、重み、またはパラメータへの不正アクセスまたは持ち出しは、IP の盗難、複製、または侵害されたバージョンの悪意のある再配布につながる可能性があります。

**推奨事項:** モデル リポジトリとサービス エンドポイントに厳格なアクセス制御を適用し、保存時の重みとチェック ポイントを暗号化し、不正な持ち出しや複製を監視します。

##### モデルの毒性 / 誤情報 / 過剰な主体性 (T01-MTU、T01-MIS、T01-EA)

**マッピングされた CWE:** [CWE-345](https://cwe.mitre.org/data/definitions/345.html)、[CWE-1204](https://cwe.mitre.org/data/definitions/1204.html)、[CWE-284](https://cwe.mitre.org/data/definitions/284.html)

**根拠:** モデルは、バイアスのある情報、有害な情報、または虚偽の情報を生成したり、有害または欺瞞的な出力に基づいて自律的なアクションを実行したりすることで、評判、倫理、または運用上の損害を引き起こす可能性があります。

**推奨事項:** 毒性およびバイアスの事後フィルターを統合します。モデル出力を検証済みの情報源に基づいて検証します。ポリシー適用によって実行可能な出力を制限します。高リスクの自律的なアクションには承認を求めます。
